...
| --- | --- | --- | --- |
| `serial_biot_savart` | Serial Biot-Savart calculation with element inputs. | `element_centers` (numpy.ndarray): (N, 3) array of center coordinates of current elements.<br>`element_lengths` (numpy.ndarray): (N,) array of lengths of current elements (dl).<br>`element_directions` (numpy.ndarray): (N, 3) array of unit vectors for current directions.<br>`field_points` (numpy.ndarray): (M, 3) array of field point coordinates.<br>`order` (int, optional): The maximum order of the Taylor series to compute. Defaults to `None`.<br>`backend` (str, optional): The backend to use ('python', 'cpp', 'c', 'cpp_v2'). Defaults to `'python'`. | `numpy.ndarray`: (M, 3) array of magnetic field vectors at each field point. |
| `mpi_biot_savart` | Parallel Biot-Savart calculation using mpi4py with element inputs. | `element_centers` (numpy.ndarray): (N, 3) array of center coordinates of current elements.<br>`element_lengths` (numpy.ndarray): (N,) array of lengths of current elements (dl).<br>`element_directions` (numpy.ndarray): (N, 3) array of unit vectors for current directions.<br>`field_points` (numpy.ndarray): (M, 3) array of field point coordinates.<br>`order` (int, optional): The maximum order of the Taylor series to compute. Defaults to `None`. | `numpy.ndarray` or `None`: On MPI rank 0, an (M, 3) array of magnetic field vectors. On other ranks, `None`. |
| `numpy_biot_savart` | NumPy vectorized Biot-Savart calculation using element inputs. | `element_centers` (numpy.ndarray): (N, 3) array of center coordinates of current elements.<br>`element_lengths` (numpy.ndarray): (N,) array of lengths of current elements (dl).<br>`element_directions` (numpy.ndarray): (N, 3) array of unit vectors for current directions.<br>`field_points` (numpy.ndarray): (M, 3) array of field point coordinates. | `numpy.ndarray`: (M, 3) array of magnetic field vectors at each field point. |
...
| :--- | :--- | :--- | :--- |
| `__init__` | Initializes the base Coil. | `current` (float or mtf.MultivariateTaylorFunction): The current flowing through the coil.<br>`use_mtf_for_segments` (bool): Whether to use MTF for segments.<br>`wire_thickness` (float): The thickness of the wire in meters. | `None` |
...
| `biot_savart` | Calculates the magnetic field generated by the coil. | `field_points` (np.ndarray or np.ndarray of mtf.MultivariateTaylorFunction): The points where the magnetic field should be calculated.<br>`backend` (str, optional): The backend to use for the calculation. | `Bfield`: A `Bfield` object containing the calculated magnetic field. |
...
| Method | Description | Arguments | Returns |
| :--- | :--- | :--- | :--- |
| `__init__` | Initializes a circular coil. | `current` (float or mtf.MultivariateTaylorFunction): The current.<br>`radius` (float): Radius of the coil.<br>`num_segments` (int): Number of segments for discretization.<br>`center_point` (np.ndarray): (3,) array for the center coordinates.<br>`axis_direction` (np.ndarray): (3,) array for the axis direction.<br>`use_mtf_for_segments` (bool): Whether to use MTF for segments.<br>`wire_thickness` (float): The thickness of the wire in meters. | `None` |
...
| Method | Description | Arguments | Returns |
| :--- | :--- | :--- | :--- |
| `__init__` | Initializes a rectangular coil. | `current` (float or mtf.MultivariateTaylorFunction): The current.<br>`p1` (np.ndarray): First corner of the rectangle.<br>`p2` (np.ndarray): Second corner, defining the first side from p1.<br>`p4` (np.ndarray): Fourth corner, defining the second side from p1.<br>`num_segments_per_side` (int): Segments per side.<br>`use_mtf_for_segments` (bool): Whether to use MTF for segments.<br>`wire_thickness` (float): The thickness of the wire in meters. | `None` |
...
| `dot` | Calculates the dot product with another Vector object. | `other` (Vector): The Vector object to take the dot product with. | `mtf.MultivariateTaylorFunction` or `float`: The scalar result of the dot product. |
...
| `norm` | Calculates the magnitude (L2-norm) of the vector. | `None` | `float` or `mtf.MultivariateTaylorFunction`: The scalar magnitude. |
...
Represents the magnetic field vector at a point.

| Method | Description | Arguments | Returns |
| :--- | :--- | :--- | :--- |
| `__init__` | Initializes the B-field vector. | `Bx` (mtf.MultivariateTaylorFunction or float): The x-component of the field vector.<br>`By` (mtf.MultivariateTaylorFunction or float): The y-component of the field vector.<br>`Bz` (mtf.MultivariateTaylorFunction or float): The z-component of the field vector. | `None` |
...
| `divergence` | Calculates the divergence of the B-field. | `None` | `mtf.MultivariateTaylorFunction`: A single MTF representing the scalar divergence of the field. |
