...
@pytest.mark.parametrize(
    "backend", ["python", "c", pytest.param("cpp", marks=pytest.mark.xfail)]
)
def test_biot_savart_ring_on_axis(backend):
    ...
    ring_coil = RingCoil(
        current, radius, num_segments, center, axis, use_mtf_for_segments=False
    )
    ...
    b_field = ring_coil.biot_savart(field_points, backend=backend)

    # The biot_savart method returns a Bfield object.
    # The _get_numerical_data() method returns a tuple of (points, vectors)
    _, b_vectors_numerical = b_field._get_numerical_data()
    ...
    assert np.allclose(
        np.real(b_vectors_numerical[:, 2].flatten()), b_z_analytical, rtol=1e-12
    )

    # Check that the imaginary part of the z-component is negligible
    assert np.allclose(np.imag(b_vectors_numerical[:, 2].flatten()), 0, atol=1e-12)

    # Check that both the real and imaginary parts of the x and y components are negligible
    assert np.allclose(b_vectors_numerical[:, 0].flatten(), 0, atol=1e-12)
    assert np.allclose(b_vectors_numerical[:, 1].flatten(), 0, atol=1e-12)
...
def test_biot_savart_with_mtf(backend):
    ...
    ring_coil = RingCoil(
        current_mtf, radius, num_segments, center, axis, use_mtf_for_segments=True
    )

    # Calculate the B-field
    b_field = ring_coil.biot_savart(field_point, backend=backend)
