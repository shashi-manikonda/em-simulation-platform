    ...
    def plot(self, ax=None, color="#B87333", num_interpolation_points=2, wire_thickness=None, show_axis=False):
        ...
        if self.use_mtf_for_segments and num_interpolation_points > 2:
            ...
            for center_vec_mtf in self.segment_centers:
                ...
                evaluated_points = np.array([x.neval(u_points) for x in center_vec_mtf]).T
                plot_ax.plot(evaluated_points[:, 0], evaluated_points[:, 1], evaluated_points[:, 2], color=color, linewidth=line_width)
        else:
        ...
        if show_axis:
            ...
            if isinstance(self, RingCoil):
                ...
                plot_ax.plot(x_vals, y_vals, z_vals, color='gray', linestyle='--', linewidth=0.8)
                ...
                warnings.warn(f"Coil type {type(self).__name__} does not have a well-defined axis for plotting.")
        ...
        **kwargs
        ...
        if plot_type == 'quiver':
            ...
            if field_component == 'Bnorm':
                ...
                field_data = np.array([b.Bx.get_constant() for b in bfield._b_vectors_mtf], dtype=float)
            elif field_component == 'By':
                field_data = np.array([b.By.get_constant() for b in bfield._b_vectors_mtf], dtype=float)
            else: # Bz
                field_data = np.array([b.Bz.get_constant() for b in bfield._b_vectors_mtf], dtype=float)
...
class RingCoil(Coil):
    ...
    def __init__(self, current, radius, num_segments, center_point, axis_direction, 
                 use_mtf_for_segments=True, wire_thickness=0.001):
        """
        ...
        if not isinstance(axis_direction, (np.ndarray, list)) or len(axis_direction) != 3:
...
class RectangularCoil(Coil):
    ...
    def generate_geometry(p1, p2, p4, num_segments_per_side, use_mtf_for_segments=True):
        ...
        for i in range(4):
            ...
            end_p = corners[(i + 1) % 4]
            all_segments.append(
                StraightWire.generate_geometry(start_p, end_p, num_segments_per_side, use_mtf_for_segments)
            )
...
class StraightWire(Coil):
    ...
    """
    def __init__(self, current, start_point, end_point, num_segments=1,
                 use_mtf_for_segments=True, wire_thickness=0.001):
        """
        ...
        self.segment_centers, self.segment_lengths, self.segment_directions = self.generate_geometry( 
    ...
    @staticmethod
    def generate_geometry( start_point, end_point, num_segments=1,
                 use_mtf_for_segments=True):
        """
