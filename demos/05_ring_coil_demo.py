...
def analytic_b_field_on_axis(z, R, I, mu0=4 * np.pi * 1e-7):
    """
...
def run_verification():
    ...
    # Analytic Solution
    B_analytic = analytic_b_field_on_axis(z_points, R, I)
    ...
    print(f"{'z(m)':<10} {'Analytic(T)':<15} {'Python(T)':<15} {'C++(T)':<15} {'Error(%)':<10}")
    print("-" * 70)
    ...
    for i, z in enumerate(z_points):
        err_py = abs(B_py[i] - B_analytic[i]) / (B_analytic[i] + 1e-15) * 100
        err_cpp = abs(B_cpp[i] - B_analytic[i]) / (B_analytic[i] + 1e-15) * 100 if cpp_success else 100.0
        
        max_error_py = max(max_error_py, err_py)
        max_error_cpp = max(max_error_cpp, err_cpp)
        
        print(f"{z:<10.2f} {B_analytic[i]:<15.6e} {B_py[i]:<15.6e} {B_cpp[i]:<15.6e} {err_py:<10.2f}")
    ...
    plt.xlabel('Z (m)')
    plt.ylabel('Bz (T)')
    plt.title(f'RingCoil On-Axis Field (R={R}m, I={I}A)')
    plt.legend()
    plt.grid(True)
