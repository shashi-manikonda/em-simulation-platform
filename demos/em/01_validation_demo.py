...
1.  **Validation:** It validates the numerical calculation of the magnetic
    B-field for a finite straight wire against the known analytical solution.
2.  **Visualization:** It demonstrates how to plot the 3D geometry of various
    current-carrying sources, such as a `StraightWire` and a `RectangularCoil`.
"""
...
#
# For a finite straight wire carrying a current `I`, the magnetic field
# magnitude at a perpendicular distance `a` from the wire is given by:
#
# $$ B_\\phi = \\frac{\\mu_0 I}{4 \\pi a} (\\cos{\\theta_1} - \\cos{\\theta_2}) $$
#
# where `z1` and `z2` are the start and end points of the wire along the
# z-axis, and the observation point is at `(a, 0, z)`.
...
#
# Here, we define a `StraightWire` object and an observation point. We then
# compute the B-field using both our numerical solver and the analytical
# formula. Finally, we calculate the relative error between the two methods to
# validate our numerical implementation.

current = 1.0
start_point = [0, 0, -1]
end_point = [0, 0, 1]
wire = StraightWire(current, start_point, end_point, num_segments=10)
...
numerator = abs(b_field_numerical_mag - b_field_analytical_mag)
error = numerator / b_field_analytical_mag
print(f"Relative error: {error:.2%}")

# ## Part 2: Geometry Visualization
#
# This section demonstrates the plotting capabilities. We visualize the
# `StraightWire` defined earlier and also create and plot a `RectangularCoil`.
...
p2 = np.array([1, 0, 0])
p4 = np.array([0, 1, 0])
rect_loop = RectangularCoil(1.0, p1, p2, p4, 5)
